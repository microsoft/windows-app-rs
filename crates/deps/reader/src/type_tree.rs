...
impl TypeTree {
    ...
    pub fn features(
        ...
        self.types
            ...
            .flatten()
            .for_each(|def| def.features(features, keys));
    }
    ...
    pub fn get_namespace(&self, namespace: &str) -> Option<&Self> {
        if let Some(next) = namespace.find('.') {
            self.namespaces
                .get(&namespace[..next])
                .and_then(|child| child.get_namespace(&namespace[next + 1..]))
        } else {
    ...
    pub fn get_namespace_mut(&mut self, namespace: &str) -> Option<&mut Self> {
        ...
        if let Some(next) = namespace.find('.') {
            self.namespaces
                .get_mut(&namespace[..next])
                .and_then(|child| child.get_namespace_mut(&namespace[next + 1..]))
        } else {
