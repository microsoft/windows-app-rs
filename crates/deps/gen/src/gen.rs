...
impl Gen {
    ...
    pub fn gen_cfg_doc(&self, features: &BTreeSet<&'static str>) -> TokenStream {
        ...
        let mut tokens = format!(
            "`{}`",
            self.relative[self.root.len() + 1..].replace('.', "_")
        );
        ...
        for feature in features {
            ...
            let feature =
                if feature.starts_with(self.root) && feature[self.root.len()..].starts_with('.') {
...
mod tests {
    ...
    fn test_features() {
        ...
        assert_eq!(
            ...
            .gen_cfg(&features)
            .as_str(),
            r#"#[cfg(feature = "Windows_Foundation")]"#
        );
        assert_eq!(
            Gen {
                root: "Microsoft",
